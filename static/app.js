        ...
        return new Promise((resolve) => {
          ...
          if (defaultValue != null) {
            $("promptBody").innerHTML =
              '<input type="text" id="promptModalInput" value="' +
              String(defaultValue).replace(/"/g, "&quot;") +
              '">';
          } else {
            $("promptBody").innerHTML =
              '<p class="' + (danger ? "danger-text" : "") + '">' + (body || "") + "</p>";
          }
  ...
  tap_to_upload: "点击上传图片",
      ...
      $("cropConfirm").addEventListener("click", () => {
        ...
        const hasEmpty = Array.from({ length: sz * sz }, (_, i) => mData[i * 4]).some(
          (v) => v > 128,
        );
      ...
      function saveSession() {
        try {
          ...
          if (lastCropRegion)
            sessionStorage.setItem("lastCropRegion", JSON.stringify(lastCropRegion));
...
$("upscaleRunBtn").addEventListener("click", async () => {
  if (!upscaleFileB64) { showError(t("error_no_image") || "No image"); return; }
  ...
  if (!preset) { showError(t("error_upscaler_path")); return; }
  const btn = $("upscaleRunBtn");
  btn.disabled = true;
  btn.textContent = t("upscaling") || "Upscaling…";
  try {
    const payload = { image: upscaleFileB64, width: w, height: h, upscaler_path: preset.path, use_opencl: false };
    const ldUrl = $("ldUrl"), ldToggle = $("ldToggle");
    if (ldToggle && ldToggle.checked && ldUrl && ldUrl.value.trim()) payload.local_dream_url = ldUrl.value.trim();
    const res = await fetch("/upscale", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  ...
  } catch (e) { showError(t("error_upscale") + ": " + e.message); }
  finally { btn.disabled = false; btn.textContent = t("upscale") || "Upscale ×4"; }
});
...
let origImgWidth = 0, origImgHeight = 0;
...
// 避免 bind() 职责膨胀；调用方一次性 dispatch input 触发即可）。
        ...
        if (evt === "progress") {
          ...
          function stitchFullToOriginal(genB64) {
            ...
            const tryShow = () => {
              ...
              if (img2imgPaddingOffset) {
                // 填充案例：gen 即 origin（576×1024），1:1 直接覆盖
                const gw = gen.naturalWidth, gh = gen.naturalHeight;
            ...
            base.onload = tryShow; base.src = "data:image/png;base64," + originalImgB64;
            gen.onload = tryShow; gen.src = genB64;
          }
          // 重置 upscale 按钮状态
      ...
      function padImageToSquare(base64, targetSize) {
        return new Promise((resolve) => {
          const img = new Image();
          img.onload = () => {
            if (img.width === img.height) { resolve(base64); return; }
      ...
      async function sendToImg2img() {
        ...
        // 保存原始图（ unpadded ）供 Download Full 使用
        originalImgB64 = b64;
        ...
        if (isNonSquare) {
          ...
          img2imgPaddingOffset = { x: Math.round((1024 - w) / 2), y: Math.round((1024 - h) / 2), origW: iw, origH: ih };
        } else {
          img2imgPaddingOffset = null;
        }
        ...
        const snap = {
          ...
          seed: data.seed != null ? data.seed : (p.seed != null ? p.seed : null),
        ...
        paddedImg.onload = () => {
          ...
          cropImg = paddedImg;
          lastCropRegion = { cropX: 0, cropY: 0, cropScale: 1, cropImgW: paddedImg.naturalWidth, cropImgH: paddedImg.naturalHeight, targetSize: 1024 };
        };
      ...
      async function compositeInpaint(origB64, genB64, maskB64) {
        ...
        // 高斯模糊遮罩，让合成边缘平滑过渡
        function featherMask(maskData, w, h, radius) {
          const d = new Float32Array(w * h);
          // 取 alpha 通道作为灰度
          for (let i = 0; i < w * h; i++) d[i] = maskData.data[i * 4] / 255;
          // 水平 passes
          ...
          for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
              ...
              for (let k = -radius; k <= radius; k++) {
                ...
                const g = Math.exp(-0.5 * (k * k) / (radius * radius));
                sum += d[y * w + xx] * g;
                wt += g;
              }
              tmp[y * w + x] = sum / wt;
        ...
        if (img2imgPaddingOffset) {
          ...
          const padded = 1024;
          const ox = img2imgPaddingOffset.x, oy = img2imgPaddingOffset.y;
          const off = document.createElement("canvas");
          off.width = padded;
          off.height = padded;
          const ctx = off.getContext("2d");
          ...
          const gw = genImg.naturalWidth, gh = genImg.naturalHeight;
        ...
        if (rawUploadedImg && lastCropRegion) {
          ...
          drawX = 0; drawY = 0; drawW = ow; drawH = oh;
        }
      ...
      $("hfToken").addEventListener("input", () =>
        localStorage.setItem("hfToken", $("hfToken").value),
      );
      ...
      function checkHealth() {
        const url =
          $("ldToggle").checked && $("ldUrl").value.trim()
            ? "/health?url=" +
              encodeURIComponent(
                $("ldUrl").value.trim().startsWith("http")
                  ? $("ldUrl").value.trim()
                  : "http://" + $("ldUrl").value.trim(),
              )
            : "/health";
        fetch(url)
          .then((r) => r.json())
      ...
      function switchLanguage(lang) {
        ...
        if (tLabel)
          tLabel.textContent = isOrig ? t("theme_label_original") : t("theme_label_apple");
